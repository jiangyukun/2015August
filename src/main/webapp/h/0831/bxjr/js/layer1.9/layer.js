...
var $, win, ready = {
    getPath: function(){
        ...
        return (window.hxb&&window.hxb.ctx?window.hxb.ctx:'/') + 'js/layer1.9/';
    }(), 
    config: {}, end: {},
    btn: ['&#x786E;&#x5B9A;','&#x53D6;&#x6D88;'],
...
window.layer = {
    ...
    use: function(module, fn, readyMethod){
        ...
        var _path = /^http:\/\//.test(module) ? module : layer.path + module;
        if(iscss){
            node.rel = 'stylesheet';
            node.href = _path;
        }else{
...
Class.pt.creat = function(){
    ...
    var layero = $('#'+ doms[0] + that.index),
        firstBtn = layero.find('.'+doms[6]+'0'),
        lastBtn = layero.find('.'+doms[6]+'1');
    
    if(typeof firstBtn[0] != 'undefined' && typeof lastBtn[0] == 'undefined'){
        firstBtn.focus();
        firstBtn.blur(function(){this.focus()});
    }
    
    if(typeof firstBtn[0] != 'undefined' && typeof lastBtn[0] != 'undefined'){
        firstBtn.focus();
        firstBtn.blur(function(){lastBtn.focus()});
        lastBtn.blur(function(){firstBtn.focus()});
    }
    
    //修复IE8下一个让人难以理解的bug
		if(parseInt(layero.css('top')) < 0){
    	layero.css('top',($("html").height() / 2 - layero.height() / 2) + 'px');
  	}
  	if(parseInt(layero.css('left')) < 0){
    	layero.css('left',($("html").width() / 2 - layero.width() / 2) + 'px');
  	}
    that.move().callback();
};
