			...
			_validateFormWithAjax: function(form, options) {
                ...
                $.ajax({
					...
					beforeSend: function() {
                        ...
                        if(submitbtn){ submitbtn.attr("disabled","disabled")}
					},
					...
					success: function(json) {
						if(typeof(json.responseText) === 'string' && /\<401/.test(json.responseText)){
							layer.confirm('表单提交遇到问题，现在需要刷新页面！',
								{ btn: ['刷新一下','算了'], icon: 3},
								function(){
									location.href = (hxb.ctx ? hxb.ctx : '')+"login/index.html";
								});
							return;
						}
                        if(submitbtn){ submitbtn.removeAttr("disabled") }
                        ...
                        if(typeof(json) === 'object' && json.link){
							if(typeof layer != 'undefined' && layer.open){
								layer.confirm(json.message, {icon: 3}, function(){ location.href = json.link; });
							}else{
								if(confirm(json.message) == true) { location.href = json.link; }
							}
                            return;
                        }
...
//                                    errorInForm|=true;
                        ...
                        if(dataType == "json" && json != 'true' && json != true){
                            ...
                            if(message){
                                layer.alert(message, {icon: success ? 1 : 2});
                            }else{
                    ...
                    if(typeof(json) == 'object' && json != 'true' && json != true){
                        ...
                        if(message){
                            var me_alert =layer.alert(message, {icon: success ? 1 : 2},function(){
                                ...
                                setTimeout(function(){callback(success,  json);},0); //callback始终延后执行
                            });
                        }else{
